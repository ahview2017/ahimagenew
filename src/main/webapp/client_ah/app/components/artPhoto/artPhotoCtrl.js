clientModule.controller('artPhotoCtrl', function ($scope, $cookies, req, md5, $state, $rootScope, $timeout, jugeGroupPos) {
    var vm = this;
    ...
    function req_getClientGroups(signId,limit,picType,size) {
        req.post('getPicture/getClientGroups.do', {
            ...
            if (resp.code == '211') {
                //安徽画坛
                if (signId == 3101) {
                    vm.anhuiPaint = jugeGroupPos.jugeGroupPos(3101,resp.data);
                }
				//网上展厅
				if (signId == 3102) {
                    vm.onlineExhibition = jugeGroupPos.jugeGroupPos(3102,resp.data);
                }
				//漫画天地
				if (signId == 3103) {

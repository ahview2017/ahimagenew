clientModule.controller('newsPhotoCtrl', function ($scope, $cookies, req, md5, $state, $rootScope, $timeout, jugeGroupPos) {
    var vm = this;
    ...
    function req_getClientGroups(signId,limit,picType,size) {
        req.post('getPicture/getClientGroups.do', {
            ...
            if (resp.code == '211') {
				...
				//时政
				if (signId == 3068) {
                    vm.politics = jugeGroupPos.jugeGroupPos(3068,resp.data);
                }
				//经济
				if (signId == 3069) {
                    vm.economic = jugeGroupPos.jugeGroupPos(3069,resp.data);
                }
				//社会
				if (signId == 3070) {
                    vm.society = jugeGroupPos.jugeGroupPos(3070,resp.data);
                }
				//科教文卫
				if (signId == 3072) {
                    vm.scienceEducation = jugeGroupPos.jugeGroupPos(3072,resp.data);
                }
				//生态
				if (signId == 3124) {
                    vm.ecosystem = jugeGroupPos.jugeGroupPos(3124,resp.data);
                }

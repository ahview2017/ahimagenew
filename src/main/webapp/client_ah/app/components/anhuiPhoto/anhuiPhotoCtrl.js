clientModule.controller('anhuiPhotoCtrl', function ($scope, $cookies, req, md5, $state, $rootScope, $timeout, jugeGroupPos) {
    var vm = this;
    ...
    function req_getClientGroups(signId,limit,picType,size) {
        req.post('getPicture/getClientGroups.do', {
            ...
            if (resp.code == '211') {
				...
				//创新安徽
				if (signId == 3090) {
                    vm.innovationAnhui = jugeGroupPos.jugeGroupPos(3090,resp.data);
                }
				//开放安徽
				if (signId == 3092) {
                    vm.openingAnhui = jugeGroupPos.jugeGroupPos(3092,resp.data);
                }
				//人文安徽
				if (signId == 3093) {
                    vm.humanitiesAnhui = jugeGroupPos.jugeGroupPos(3093,resp.data);
                }

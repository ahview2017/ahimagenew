clientModule.controller('historyPhotoCtrl', function ($scope, $cookies, req, md5, $state, $rootScope, $timeout, jugeGroupPos) {
    var vm = this;
    ...
    function req_getClientGroups(signId,limit,picType,size) {
        req.post('getPicture/getClientGroups.do', {
            ...
            if (resp.code == '211') {
                //历史图片
                if (signId == 3094) {
                    vm.historicalPictures = jugeGroupPos.jugeGroupPos(3094,resp.data);
                }
				//新华图片
				if (signId == 3095) {
                    vm.xinhuaPictures = jugeGroupPos.jugeGroupPos(3095,resp.data);
                }
				//旧报影印
				if (signId == 3096) {
                    vm.oldNewspaper = jugeGroupPos.jugeGroupPos(3096,resp.data);
                }			
				//影像江淮
				if (signId == 3097) {
                    vm.newspaperPictures = jugeGroupPos.jugeGroupPos(3097,resp.data);
                }

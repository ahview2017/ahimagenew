clientModule.controller('videoVRCtrl', function ($scope, $cookies, req, md5, $state, $rootScope, $timeout, jugeGroupPos) {
    var vm = this;
    ...
    function req_getClientGroups(signId,limit,picType,size) {
        req.post('getPicture/getClientGroups.do', {
            ...
            if (resp.code == '211') {
                //航拍视频
                if (signId == 3098) {
                    vm.aerialVideo = jugeGroupPos.jugeGroupPos(3101,resp.data);
                }
				//微视频库
				if (signId == 3099) {
                    vm.microVideo = jugeGroupPos.jugeGroupPos(3102,resp.data);
                }
				//VR
				if (signId == 3100) {
                    vm.VR = jugeGroupPos.jugeGroupPos(3103,resp.data);
                }

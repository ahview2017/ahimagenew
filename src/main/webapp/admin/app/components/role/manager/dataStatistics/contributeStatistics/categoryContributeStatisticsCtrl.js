 ...
 */
adminModule.controller('categoryContributeStatisticsCtrl', function ($scope, $cookies, req, md5, $state, $rootScope) {
    var vm = this;
    //初始化相关配置
    ...
    function getContributorTableData(page,type) {
        var searchUrl = "groupStatistical/groupStatisticalForCategoryList.do";
        var paramsObj = {
            page: page,
            rows: 10
        };
        if (vm.categoryName) {
            paramsObj['categaryName'] = vm.categoryName;
        } 
        if(vm.orderByCase){
